...
function MoodTrendChart({ points, labels }: { points: number[]; labels: string[] }) {
  const width = 400;
  const height = 120;
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} className="h-full w-full" preserveAspectRatio="xMidYMid meet" aria-hidden>
      <defs>
        <linearGradient id="mood-fill" x1="0" y1="0" x2="0" y2="1" gradientUnits="objectBoundingBox">
      ...
      <polyline points={linePoints} fill="none" stroke="rgb(108, 43, 238)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
      {coords.map((c, i) => (
        <circle key={i} cx={c.x} cy={c.y} r="4" fill="rgb(108, 43, 238)" />
      ))}
      {labels.map((label, i) => (
        <text key={label + i} x={padding.left + i * stepX} y={height - 6} textAnchor="middle" className="fill-slate-400 text-[10px] font-medium">
          {label}
        </text>
      ))}
    </svg>
...
export default function DashboardPage() {
  ...
  return (
    <div className="font-sans text-slate-800">
      <h1 className="mb-2 font-headline text-3xl font-bold text-slate-800 md:text-4xl">Good morning, Alex.</h1>
      <p className="mb-8 text-lg text-slate-600">Where shall we go within today?</p>
      ...
      <div className="mb-8 grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
        <motion.div
          ...
          className="relative flex min-h-[170px] w-full flex-col overflow-hidden rounded-3xl border border-dashboard-stroke bg-white shadow-card-layered md:min-h-[190px]"
        >
          <div
            ...
            aria-hidden
          />
          <div className="absolute inset-0 bg-gradient-to-r from-white/95 via-white/80 to-brand/20" aria-hidden />
          ...
          <div className="relative z-10 flex flex-1 flex-col justify-between px-6 py-5 md:px-7 md:py-6">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold uppercase tracking-wider text-slate-500">Daily Prompt</span>
            </div>

            <div className="mt-4">
              <h2 className="mb-3 font-headline text-2xl font-bold text-slate-800 md:text-3xl">Welcome</h2>
              <p className="font-headline text-lg italic leading-relaxed text-slate-700">
                &ldquo;Your visions will become clear only when you can look into your own heart.&rdquo;
              </p>
              <p className="mt-2 text-xs font-semibold uppercase tracking-wider text-slate-500">— Carl Jung</p>
            </div>
        ...
        <div className="flex flex-col gap-6">
          <div className="min-w-0 overflow-hidden rounded-2xl border border-dashboard-stroke bg-white px-4 py-5 shadow-card-layered">
            <div className="mb-4 flex items-center justify-between gap-2">
              <h2 className="shrink-0 font-sans text-sm font-bold text-slate-900">Weekly Streak</h2>
              <span className="shrink-0 rounded-full bg-brand/10 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide text-brand">5 days</span>
            </div>
                  ...
                  <div key={day + i} className="flex flex-col items-center gap-1">
                    <div
                      className={`flex h-5 w-5 items-center justify-center rounded-full text-[9px] font-medium sm:h-6 sm:w-6 sm:text-[10px] ${
                        filled ? "bg-brand text-white" : "border border-slate-200 bg-slate-50 text-slate-400"
                      }`}
                    >
                      {filled ? <Check className="h-3 w-3" /> : day}
                    </div>
                    <span className="truncate text-[9px] font-medium text-slate-500 sm:text-[10px]">{day}</span>
                  </div>
                );
              })}
            </div>
          ...
          <div>
            ...
            <div className="pointer-events-none select-none divide-y divide-dashboard-stroke overflow-hidden rounded-2xl border border-dashboard-stroke bg-white/70 opacity-60 shadow-card-layered backdrop-blur-md">
              <div className="flex items-start gap-3 p-4">
                ...
                <div>
                  <p className="text-xs font-semibold uppercase tracking-wider text-slate-400">Dominant archetype</p>
              ...
              <div className="flex items-start gap-3 p-4">
                ...
                <div>
                  <p className="text-xs font-semibold uppercase tracking-wider text-slate-400">Shadow focus</p>
                  <p className="mt-1 font-semibold text-slate-500">Professional Envy</p>
                  <p className="text-xs text-slate-400">Primary area of integration</p>
                </div>
              </div>
              <div className="p-4">
                <div className="mb-2 flex items-center gap-2">
                  <Star className="h-4 w-4 text-slate-500" />
                  <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">AI observation</p>
                </div>
                <p className="text-sm italic text-slate-500">
                  Your recent entries suggest a transition from the &lsquo;Anima&rsquo; projection to personal empowerment.
                  Consider writing about your childhood mentors next.
                </p>
              </div>
              <p className="py-3 text-center text-xs text-slate-400">Premium feature — coming soon</p>
            </div>
          </div>
        </div>
      </div>

      <div className="mb-8 rounded-2xl border border-dashboard-stroke bg-white p-6 shadow-card-layered">
        ...
        <div className="rounded-2xl border border-dashboard-stroke bg-white p-4 shadow-card-layered">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-headline text-2xl font-bold text-slate-900">12</p>
            </div>
          </div>
        </div>
        <div className="rounded-2xl border border-dashboard-stroke bg-white p-4 shadow-card-layered">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-headline text-2xl font-bold text-slate-900">8,240</p>
            </div>
          </div>
        </div>
        <div className="rounded-2xl border border-dashboard-stroke bg-white p-4 shadow-card-layered">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-headline text-lg font-bold text-slate-900">Thursday</p>
            </div>
          </div>
        </div>
      </div>
